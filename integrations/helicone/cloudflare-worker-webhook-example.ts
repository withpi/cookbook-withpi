// cloudflare-worker-webhook-example.ts
...
// Define the environment variables interface for the Worker
export interface Env {
  WITHPI_API_KEY: string;
  HELICONE_API_KEY: string;
  HELICONE_WEBHOOK_SECRET: string;
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (!env.HELICONE_WEBHOOK_SECRET) {
        throw new Error('HELICONE_WEBHOOK_SECRET is not configured');
      }

      // Verify HMAC signature
      const isAuthenticated = await verifyHeliconeHmac(
        request.headers,
        rawBody,
        env.HELICONE_WEBHOOK_SECRET,
      );
      if (!isAuthenticated) {
        return new Response(
          JSON.stringify({
      ...
      // Send the POST request to WithPI
      const piApiResponse = await fetch(WITHPI_API_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-api-key': env.WITHPI_API_KEY,
        },
      ...
      return new Response(
        JSON.stringify({
          status: 'error',
          message: error.message || 'Failed to process scoring',
        }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
